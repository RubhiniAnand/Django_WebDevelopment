...
class dataUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            data_male=request.POST.get('sex_male')
            data_yes=request.POST.get('smoker_yes')
            #print (data)
            #dataset1=pd.read_csv("prep.csv",index_col=None)
            #dicc={'yes':1,'no':0}m,bbbbbbjs6t7y8u6uuuuu
            filename = 'finalized_model_RF.sav'
            classifier = pickle.load(open(filename, 'rb'))

            data = np.array([data_age,data_bmi,data_children,data_male,data_yes])
            #sc = StandardScaler()
...
#data_for_prediction =obj.pca(np.array(data_for_prediction),y_test)
            ...
            return render(request, "succ_msg.html", {'data_age':data_age,'data_bmi':data_bmi,'data_children':data_children,'data_male':data_male,'data_yes':data_yes,
                                                        'out':out})
